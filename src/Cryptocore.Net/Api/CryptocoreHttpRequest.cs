...
{
    public class CryptocoreHttpRequest
    {
        ...
        private const string RequestHeaderKeyName = "X-CryptoCore-ApiKey";
        ...
        public CryptocoreHttpRequest(string path)
        {
        ...
        public void AddParameter<T>(string field, T value)
        {
            ...
            if (_parameters.ContainsKey(field))
                throw new InvalidOperationException($"{nameof(CryptocoreHttpRequest)}: request already has a '{field}' parameter.");

            if (_parameters.ContainsKey("signature"))
                throw new InvalidOperationException($"{nameof(CryptocoreHttpRequest)}: all parameters must be added before request is signed.");
        ...
        public HttpRequestMessage CreateMessage(HttpMethod method)
        {
            ...
            if (method == HttpMethod.Get && Body != null)
                throw new InvalidOperationException($"{nameof(CryptocoreHttpRequest)}: parameters must be sent in query string for GET requests ({nameof(Body)} must be null).");
